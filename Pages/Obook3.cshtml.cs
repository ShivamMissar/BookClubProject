...
{
    public class Obook3Model : PageModel
    {
        ...
        public Obook3Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("Mageborn")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("Mageborn")).ToList();
        }

        public IActionResult OnPostAddReview()
        {
            const string bookType = "Mageborn";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/Obook3");
        }
