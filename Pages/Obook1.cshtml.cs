...
{
    public class Obook1Model : PageModel
    {
        ...
        public Obook1Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("TheNameOfTheWind")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("The Name Of The Wind")).ToList();
        }

        public IActionResult OnPostAddReview()
        {
            const string bookType = "The Name Of The Wind";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/Obook1");
        }
