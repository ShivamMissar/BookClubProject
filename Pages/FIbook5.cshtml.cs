...
{
    public class FI_book5Model : PageModel
    {
        ...
        public FI_book5Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("Songbirds")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("Songbirds")).ToList();
        }

        public IActionResult OnPostAddReview()
        {
            const string bookType = "Songbirds";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/FIbook5");
        }
