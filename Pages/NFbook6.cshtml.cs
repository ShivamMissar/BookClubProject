...
{
    public class book6Model : PageModel
    {
        ...
        public book6Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("DarkMatter")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("Dark Matter")).ToList();
        }


        public IActionResult OnPostAddReview()
        {
            const string bookType = "Dark Matter";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/NFbook6");
        }
