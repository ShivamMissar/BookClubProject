...
{
    public class FI_book1Model : PageModel
    {
        ...
        public FI_book1Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("ThePaperPalace")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("The Paper Palace")).ToList();
        }

        public IActionResult OnPostAddReview()
        {
            const string bookType = "The Paper Palace";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/FIbook1");
        }
