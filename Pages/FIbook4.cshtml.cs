...
{
    public class FI_book4Model : PageModel
    {
        ...
        public FI_book4Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("Theleviathan")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("The Leviathan")).ToList();
        }

        public IActionResult OnPostAddReview()
        {
            const string bookType = "The Leviathan";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/FIbook4");
        }
