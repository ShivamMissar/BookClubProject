...
{
    public class Obook2Model : PageModel
    {
        ...
        public Obook2Model(AppDataContext db)
        ...
        public void OnGet()
        {
            book = _db.Books.Where(p => p.BookImage.Contains("TheShadowOfTheGods")).FirstOrDefault();
            get_reviews = _db.Reviews.Where(p => p.bookName.Contains("The Shadow Of The Gods")).ToList();
        }

        public IActionResult OnPostAddReview()
        {
            const string bookType = "The Shadow Of The Gods";
            customer_said.bookName = bookType;
            _db.Add(customer_said);
            _db.SaveChanges();
            return RedirectToPage("/Obook2");
        }
